...
The spec calls for Clang/LibTooling. That's a native toolchain that
must be installed on the host machine (it isn't pip-installable), so
this module ships a real, working heuristic/regex analyzer as the
functional fallback (per section 33). If Clang is available on the
machine this runs on, you can shell out to `clang -fsyntax-only` for
real compiler-grade syntax errors — see the commented hook below.
"""
...
VAR_RE = re.compile(r'\b(int|double|float|long|bool|char|auto|std::\w+)\s+(\w+)\s*=')


def _try_clang_syntax_check(code: str):
    """Optional real-compiler hook: only runs if `clang++` is on PATH."""
...
def analyze(code: str) -> dict:
    ...
    result["analyzer_note"] = (
        ...
        "and wire it into backend/analyzers/cpp_analyzer.py."
    )
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if re.search(r'/\s*0\b', line):
            result["issues"].append(make_issue(i, "Possible Division by Zero",
                                     ...
                                     confirmed=False))
        if "delete" in line and re.search(r'delete\s+\w+;', line):
            result["issues"].append(make_issue(i, "Possible Dangling Pointer",
                                     "A pointer is deleted here; using it again afterwards without "
                                     "setting it to nullptr can cause a dangling-pointer bug.",
                                     confirmed=False))
